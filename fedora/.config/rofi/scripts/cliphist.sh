...
case "${main_action}" in
"History")
    selected_item=$($clipPath list | rofi -dmenu -theme-str "entry { placeholder: \"Clipboard History...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}")
    if [ -n "$selected_item" ]; then
        echo "$selected_item" | $clipPath decode | wl-copy
        notify-send "Copied to clipboard."
    fi
    ;;
"Delete")
    selected_item=$($clipPath list | rofi -dmenu -theme-str "entry { placeholder: \"Delete...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}")
    if [ -n "$selected_item" ]; then
        echo "$selected_item" | $clipPath delete
        notify-send "Deleted."
    fi
    ;;
"View Favorites")
...
"Manage Favorites")
    ...
    "Add to Favorites")
        ...
        item=$($clipPath list | rofi -dmenu -theme-str "entry { placeholder: \"Add to Favorites...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}")
        if [ -n "$item" ]; then
            # Decode the item from clipboard history
            full_item=$(echo "$item" | $clipPath decode)
